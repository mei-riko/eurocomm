...
$(document).ready(() =>{
    ...
    if( $("#mapOffice").length > 0 ){
      var mapOffice = new mapboxgl.Map({
        container: 'mapOffice',
        style: 'mapbox://styles/mapbox/light-v10',
        center: [6.1493, 46.200767],
        zoom: 14
      });
  
      var navOffice = new mapboxgl.NavigationControl();
      mapOffice.addControl(navOffice, 'top-left');
      ...
      var geojsonOffice = {
        ...
        features: [{
          ...
          geometry: {
            type: 'Point',
            coordinates: [6.1493, 46.200767]
          },
          properties: {
            title: 'Head Office',
            description: 'EURO COMM SA, 4 Place du Bourg de Four, 1204 Geneva, Switzerland'
          }
        }]
      };
      // Add markers to map
      geojsonOffice.features.forEach(function(markerOffice) {
        // create a HTML element for each feature
        var elOffice = document.createElement('div');
        elOffice.className = 'map-marker-1';

        // make a marker for each feature and add to the map
        new mapboxgl.Marker(elOffice)
            .setLngLat(markerOffice.geometry.coordinates)
            .addTo(mapOffice);
        // popup
        new mapboxgl.Marker(elOffice)
            .setLngLat(markerOffice.geometry.coordinates)
            .setPopup(new mapboxgl.Popup({ offset: 25 }) // add popups
                .setHTML('<p class="text-bold mb-0">' + markerOffice.properties.title + '</p><p class="text-small mb-0">' + markerOffice.properties.description + '</p>'))
            .addTo(mapOffice);
      });
    }
